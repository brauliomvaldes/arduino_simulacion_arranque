// RUTINA SIMULA ARRANQUE CONTACTORES UTILIZANDO LED
// Autores: Patricio Tiznado - Braulio Valdés
//
// LED ENCIENDE CON, RELE SE ACTIVA CON LOW
// CON INVERSION DE GIRO 
// ARRANQUE ESTRELLA TRIANGULO 
// SENSORES DE PARADA Y DE ARRANQUE SENTIDO CONTRARIO
//
...
void arranque(int pin){
	...
	if (estadoOperativo==false){
		digitalWrite(pin,       HIGH);			// enciende derecha o ezquierda
		digitalWrite(ESTRELLA,  HIGH);			// enciende estrella
    		estadoISR = false;
		...
		digitalWrite(ESTRELLA,  LOW);			// apaga estrella
		...
		// si ocurre una interrupcion no espera ni ejecuta triangulo
   		if (estadoISR == false){                 		// si no ocurre una interrupcion
      			digitalWrite(TRIANGULO, HIGH);		// enciende triangulo  
...
void loop() {
	// lee botones
  	...
  	if (estado1 == 1 && estadoOperativo == false){  
    		...
    		arranque(DERECHA);  		//ENCENDER LUZ
  	}
  	else{
      		if(estado1 == 0){
        		digitalWrite(DERECHA, LOW);  	// APAGAR LUZ
      		}
  	}
  	if (arranqueizq == LOW){     		// SE PRESIONO BOTON VERDE
 	  ...
 	  if (estado2 == 1 && estadoOperativo == false){
       		Serial.println("marcha <---"); 
	     	arranque(IZQUIERDA); 		//ENCENDER LUZ
	  }else{
      		if(estado2 == 0){
        		digitalWrite(IZQUIERDA, LOW);   // APAGAR LUZ
      		} 
  	}
...
void apagadoPorSensor(int pin){
   	digitalWrite(pin, LOW);     // apagar led sentido derecho/izquierdo
    	digitalWrite(TRIANGULO, LOW); // apagar led
